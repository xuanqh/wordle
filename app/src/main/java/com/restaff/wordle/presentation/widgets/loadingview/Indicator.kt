package com.restaff.wordle.presentation.widgets.loadingview
...
abstract class Indicator : Drawable(), Animatable {
    ...
    private fun startAnimators() {
        if(mAnimators!=null){
            mAnimators?.let { animationItems->
                for (i in animationItems.indices) {
                    val animator = animationItems[i]
    ...
    private fun stopAnimators() {
        mAnimators?.let { listItems ->
            for (animator in listItems) {
